...
namespace App {
    ...
    void BattleMaster::HandleActionInput(UnitBase& actor, UnitBase& targetUnit) {
        ...
        if (CheckButtonClick(600, 960, 220, 60, mousePos) || hoverGrid == pos || (CheckButtonClick(40, 100, 500, 650, mousePos) && is1P) || (CheckButtonClick(1380, 100, 500, 650, mousePos) && !is1P)) {
            ...
            AddLog("【待機】 何もせずにターン終了"); actionDone = true;
        }
        else {
            AddLog("【待機】 キャンセル (ターン終了)"); actionDone = true;
        }
        ...
        if (std::abs(myP.x - oppP.x) + std::abs(myP.y - oppP.y) == 1) {
            ...
            if (myOp != '\0') {
                if (m_ruleMode == RuleMode::ZERO_ONE) {
                    ...
                    if (myOp == '/') ExecuteBattle(*me, *opp, *me);
                    else if (delta < 0) ExecuteBattle(*me, *opp, *opp);
                    else ExecuteBattle(*me, *opp, *me);
                }
            }
            else {
    ...
    // （ExecuteAI, PerformAIMove, EvaluateBoard は内部ロジックメインなのでそのまま）
    void BattleMaster::PerformAIMove(UnitBase* me, IntVector2 bestTarget, int selectedCost, bool is1P) {
        ...
        if (me->HasWarpNode(bestTarget)) AddLog("【行動】 " + myName + " が陣地へワープ！ (コスト: -1)");
        else if (bestTarget == ePos) AddLog("【待機】 " + myName + " がその場で数字を調整 (コスト: -1)");
        else AddLog("【行動】 " + myName + " が進軍 (コスト: -" + std::to_string(selectedCost) + ")");
                ...
                if (canAttack) {
                    ...
                    if (myOp == '/') {
                        ...
                        if (delta > 0) {
                            ...
                            if (sChange > 0) score += sChange * 5000;
                            else score += delta * 50;
                        }
                    }
                }
    ...
    void BattleMaster::DrawEnemyDangerArea() const {
        ...
        for (int x = 0; x < m_mapGrid.GetWidth(); ++x) {
            for (int y = 0; y < m_mapGrid.GetHeight(); ++y) {
                ...
                if (canMoveCombined) {
                    unsigned int areaCol = canMoveBase ? COL_DANGER_DIM() : COL_INFO(); // 通常とジャンプで色分け
    ...
    bool BattleMaster::IsGameOver() const {
        ...
        // クラシックモード：残機0 かつ 体力1以下 で敗北
        // （体力1はゲーム上の最小値。0以下になる計算なら負け）
        if (m_player->GetStocks() <= 0 && m_player->GetNumber() <= 1) return true;
        if (m_enemy->GetStocks() <= 0 && m_enemy->GetNumber() <= 1) return true;
    ...
    bool BattleMaster::IsPlayerWin() const {
        ...
        if (m_ruleMode == RuleMode::ZERO_ONE) {
            ...
            return (m_enemy->GetStocks() <= 0 && m_enemy->GetNumber() <= 1);
        }
    }
